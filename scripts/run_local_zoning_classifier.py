#!/usr/bin/env python3
"""
One-command zoning classifier runner with optional auto-start gateway.
...
If --gateway-url is not supplied, this script will:
  ...
  2) wait for /healthz
  3) run scripts/run_zoning_classifier.py against your per-page *.vlm.json files
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run local zoning classifier with optional auto-start gateway.")
    p.add_argument("--pages", required=True, help='Glob or "@file" listing per-page *.vlm.json paths')
    p.add_argument("--output-dir", required=True, help="Where to write per-page classifier outputs")
    p.add_argument("--model", required=True, help="Classifier model, e.g. openai:gpt-5-nano")
    p.add_argument("--prompt-path", default="prompts/zoning_ocr_classifier_prompt_text.txt", help="Prompt file path")
    p.add_argument("--max-concurrency", type=int, default=4, help="Pages to classify concurrently")
    p.add_argument("--timeout", type=float, default=120.0, help="Gateway timeout seconds per call")
    p.add_argument("--max-retries", type=int, default=2, help="Retries per page on parse/transport error")
    p.add_argument("--skip-existing", action="store_true", help="Skip pages with existing outputs")
